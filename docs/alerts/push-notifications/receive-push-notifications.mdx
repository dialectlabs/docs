...
APIs and manage the user's subscription. The authentication process uses the
[Sign-in with Solana (SIWS)](https://github.com/phantom/sign-in-with-solana) flow, which forces users to sign a message
...
<PlatformCodeTabs
  androidCode={`FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
  ...
  iosCode={`Messaging.messaging().token { token, error in
  if let error = error {
    // Handle error
    return
  }
  ...
  webCode={`getToken(messaging, { vapidKey: 'YOUR_VAPID_KEY' })
  .then((currentToken) => {
    if (currentToken) {
      // Store FCM token for later use
      // ...
...
For detailed implementation instructions, refer to the [Firebase Cloud Messaging documentation](https://firebase.google.com/docs/cloud-messaging) for [iOS](https://firebase.google.com/docs/cloud-messaging/ios/client) and [Android](https://firebase.google.com/docs/cloud-messaging/android/client).
...
If you want to test the endpoints, visit the
[`/subscribe`](https://alerts-api.dial.to/docs#tag/subscriber/POST/v2/push/subscribe) endpoint in our API docs.
